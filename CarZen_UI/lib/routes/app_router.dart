...
/// Maps URLs to pages. Public pages (home, browse, car details, accessories,
...
class AppRouter {
  ...
  static Widget _pageFor(Uri uri) {
    ...
    switch (first) {
      ...
      case 'sell':
        return RequireAuth(child: second == 'new' ? const AddEditCarScreen() : const MyCarsScreen());
      case 'favorites':
        return const RequireAuth(child: FavoritesScreen());
      case 'orders':
        return const RequireAuth(child: MyOrdersScreen());
      case 'inquiries':
        if (segments.length == 1) return const RequireAuth(child: InquiriesScreen());
        final inquiryId = int.tryParse(segments[1]);
        return inquiryId == null
            ? const NotFoundPage()
            : RequireAuth(child: InquiryThreadScreen(inquiryId: inquiryId));
      case 'notifications':
      ...
      case 'accessories':
        return const IntegrationUnavailableScreen(
          ...
          message: 'The current CarZen backend does not expose accessory catalog or purchase APIs yet.',
          section: NavSection.accessories,
        );
      case 'login':
